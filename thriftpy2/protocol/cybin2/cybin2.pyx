...
cdef inline c_read_binary_as_bytes(CyTransportBase buf, int32_t size):
...
cdef inline c_read_binary_as_memoryview(CyTransportBase buf, int32_t size):
...
cdef c_read_val(CyTransportBase buf, TType ttype, spec=None,
    ...
    if ttype == T_BOOL:
        ...
        if decode_response:
            ...
            return c_read_binary_as_bytes(buf, size)
...
cpdef skip(CyTransportBase buf, TType ttype):
    ...
    if ttype == T_BOOL or ttype == T_I08:
        ...
        size = read_i32(buf)
        c_read_binary_as_bytes(buf, size)
    elif ttype == T_SET or ttype == T_LIST:
...
cdef class TCyBinaryProtocol2(object):
...
class TCyBinaryProtocolFactory2(object):
